...
class Cutter(object):
    ...
    def leakage_cut(self, camera, hillas_parameters=None,
                    radius=0.8, max_dist="mean", image=None,
                    rows=2, fraction=0.15, method="radius"):
        '''
        Handeling of images at the edge of the camera.

        Parameters
        ----------
        method : string
        camera : CameraDesctription
            ctapipe camera disctription
        hillas_parameters : HillasParametersContainer or None
            Container with hillas parameters stored. Required for
            method "radius
        radius : float
            maximum relative distance of cog to center of the camera
        max_size : ["mean", "max"]
            Method to calculate the total size of the camera. If "mean",
            the average distance from the boundary pixels to the center
            are considered. If "max", the distance to the pixel furthest
            away is taken.
        image : ndarray
            image in camera in pe
        rows : integer
            number of rows defining edge for fraction method
        fraction : float
            maximum fraction of charge contained in edge of camera

        Returns
        -------
        True if the cog is within accepted radius, False otherwise
        '''
        ...
        if method == "None":
            ...
            raise ValueError("Leakage method {} not known.".format(method))
    ...
    #     Returns
    #     -------
    #     True if the cog is within accepted radius, otherwiese Falses
    #     '''
    #
    #     # get the maximum distance from camera
    #     if method == "max":
