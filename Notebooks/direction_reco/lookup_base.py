...
class LookupBase:
    ...
    def look_up_value(self, params, key):
        '''
        Get the value and number of entries to a given set of parameters.
        The number of parameters passed must be equal to the number of of
        dimensions the look up table has.
        ...
        key : string
            key for selecting the entry of the lookup dictionary
        ...
        dims = np.shape(self.lookup[key])[0] - 2
        if dims != len(params):
        ...
        try:
            _bin = []
            for i in range(dims):
                _bin.append(np.arange(len(self.lookup[key][i + 1]))[
                    params[i] > self.lookup[key][i + 1]][-1])

            statistic = int(self.lookup[key][0][tuple(_bin)])
            value = self.lookup[key][-1][tuple(_bin)]
    ...
    def display_lookup(self, figsize=None, xlabel="attr_1", ylabel="attr_2",
    				   xscale="log", yscale="linear", cmap="inferno"):
        '''
        ...
        f, axarr = plt.subplots(number_entries, 2,
        						figsize=figsize)
