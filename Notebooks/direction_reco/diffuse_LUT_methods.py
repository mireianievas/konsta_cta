    ...
    for offbin in LUTs:
        if type(offbin) == int:
            for cam in LUTs[offbin]:
                maximum = np.max(LUTs[offbin][cam][-1][~np.isnan(LUTs[offbin][cam][-1])])
                minimum = np.min(LUTs[offbin][cam][-1][~np.isnan(LUTs[offbin][cam][-1])])
                try:
...
def plot_verbose(LUTs, figsize=[15, 15], cameras=None, max_val=None, min_val=None, min_stat=0):
    '''
    ...
    fig, axarr = plt.subplots(len(cameras), len(LUTs["bins"]) - 1, figsize=figsize)
        ...
        for i in LUTs:
            if type(i) == int:
                ...
                try:
                    ...
                    mask = LUTs[i][cam][0] < min_stat
                    LUT[mask] = np.NaN

                    LUT = LUT.T

                    xbins, ybins = (LUTs[i][cam][1], LUTs[i][cam][2])
                    # print(len(xbins))
                ...
                if i == len(LUTs["bins"]) - 2:
                    # ax2 = axarr[k, i].twinx()
                    # lab = ax2.set_ylabel(cam, fontsize=20, labelpad=90)
                    # ax2.set_yticks([])

                    plt.colorbar(val, ax=axarr[k, i], norm=LogNorm())
                if k == 0:
                    axarr[k, i].set_title("{}$^\circ$ - {}$^\circ$".format(int(LUTs["bins"][i]),
                                                                           int(LUTs["bins"][i + 1])), fontsize=20)
